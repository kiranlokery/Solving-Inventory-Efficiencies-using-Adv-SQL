...
-- 2. Low Inventory Detection & Reorder Alerts
...
WITH 
  ...
  coverage AS (
    SELECT
      ...
      rc.safety_stock,
      rc.reorder_point,
      ROUND(ls.inventory_level / GREATEST(rc.avg_daily_sales, 0.1), 0) AS days_of_supply,
      GREATEST(
        0,
...
-- 3. Inventory Turnover & Movement Classification
...
WITH 
  ...
  turnover_calculations AS (
    SELECT 
      ...
      ROUND(SUM(i.units_sold * i.price * (1 - i.discount/100)), 2) AS cogs_90_days,
      ROUND(AVG(i.inventory_level * i.price * (1 - i.discount/100)), 2) AS avg_inventory_value,
      CASE 
        WHEN AVG(i.inventory_level) > 0 THEN 
          ROUND((SUM(i.units_sold) * 365.0 / 90.0) / AVG(i.inventory_level), 2)
        ELSE 0 
