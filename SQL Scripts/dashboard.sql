...
SELECT * FROM vw_seasonality_summary;
...
SELECT 
    ...
    (
        SELECT category FROM category_impact ci3 
        WHERE ci3.seasonality = sp.seasonality 
        ORDER BY ci3.avg_seasonal_index ASC LIMIT 1
    ) AS lowest_impact_category,
    (
        SELECT avg_seasonal_index FROM category_impact ci3 
        WHERE ci3.seasonality = sp.seasonality 
        ORDER BY ci3.avg_seasonal_index ASC LIMIT 1
    ) AS lowest_impact_index
FROM season_performance sp
JOIN impact_counts ic ON sp.seasonality = ic.seasonality
CROSS JOIN overall_baseline ob
ORDER BY seasonal_index DESC;
